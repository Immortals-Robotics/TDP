...
When implementing each state of an FSM, the developer has to consider the other states and how they are transitioned to the current state and how it will transition to the next state. For a state machine consisting of only a few states, this can be more easily handled. However, considering the number of robots and the various tactics that the opponent can use, it is required by the developer to implement a high number of states to handle every possible situation in the FSM.
